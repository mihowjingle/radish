...
- `then` - a branch in `when` and in if expression
...
- `readonly` - inside of `slot`s; everything in `readonly` (and in `constructors`, and `mutations`) is public, everything else is private
- `this` - for an object to refer to itself and to indicate the private implementation details in its definition
- `mutations` - inside of `slot`s; for functions (methods) which mutate `this`
