...
longestByName = (this: Iterable[any N of Named]) -> Maybe[N] {
    if this.empty {
        return None
    }
    current: Variable[N] = Variable.new(this[0])
    for element in this {
        if element.name.length > current.name.length {
            current.set(element)
        }
    }
    return current.get()
...
Then the following will return `Maybe[Person]` instead of `Maybe[Named]`, if we didn't use `any N of Named`:

```
type Person of Named {
    name: String
    age: Integer
}
...
personWithLongestName: Maybe[Person] = people.longestByName()
```

This would look cool, but may be problematic:

```
Person = type of Named {
    name: String
    age: Integer
}
```