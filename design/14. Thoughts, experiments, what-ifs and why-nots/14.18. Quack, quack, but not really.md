...
Hmm, that would mean that there should never be a `type`-specific function or property...
...
_(3)_ Speaking of "is". What would be the behaviour of `is` (instance of) operator vs `type`s vs `feature`s?